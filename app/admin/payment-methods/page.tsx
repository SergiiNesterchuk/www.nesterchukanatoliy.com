      ...
      <div className="space-y-4">
        {methods.map((m) => (
          <div key={m.id} className="bg-white rounded-xl border p-6">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Інструкція для клієнта (після замовлення)</label>
              <textarea value={m.customerInstruction || ""} onChange={(e) => update(m.id, "customerInstruction", e.target.value)} rows={2} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
            </div>
