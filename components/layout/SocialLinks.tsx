...
export function SocialLinks({ links, className = "", iconSize = 20 }: SocialLinksProps) {
  const activeLinks = Object.entries(links).filter(([key, url]) => url && SOCIAL_ICONS[key]);
    ...
    <div className={`flex gap-3 ${className}`}>
      {activeLinks.map(([key, url]) => {
        ...
        return (
          ...
          >
            <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="currentColor" dangerouslySetInnerHTML={{ __html: icon.svg }} />
          </a>
        );
      })}
    </div>
